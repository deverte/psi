...
class Axes {
  ...
  static add(obj, axis) {
    if (axis == undefined) {
      ...
      axis.position = _.defaultTo(axis.position, Axis._POSITION); // if axis.position.left == undefined || axis.position.right == undefined

      _.defaultsDeep(axis.position, Axis._POSITION); // To start from the end
  ...
  static async generateLabels(obj) {
    ...
    if (obj.axis.label.text != undefined) {
      let label = new Block(obj.axis.label.text, obj.mjAPI, obj.mjTypeset, {
        format: format,
        wrapper: {
          strokeWidth: 0
        }
    ...
    for (let column of obj.columns) {
      if (column.time != undefined) {
        ...
        if (column.time.text != undefined) {
          let label = new Block(column.time.text, obj.mjAPI, obj.mjTypeset, {
            format: format,
            wrapper: {
              strokeWidth: 0
            }
  ...
  static generate(obj) {
    if (obj.axis != null) {
      let axisHeight = obj.axis.margin + obj.axis.arrowHeight * 2 + obj.axis.width + this.getLabelsMaxHeight(obj);
      obj.svg.height(obj.svg.height() + axisHeight); // Line
      ...
      let y = obj.svg.height() - obj.style.canvas.margin.bottom - this.getLabelsMaxHeight(obj) - obj.axis.arrowHeight;
      obj.svg.line(x0, y, x, y).stroke({
        color: obj.axis.color,
        width: obj.axis.width,
        linecap: 'round'
      }); // Arrow

      if (obj.axis.direction == 'left') {
        obj.svg.line(x0, y, x0 + obj.axis.arrowHeight, y + obj.axis.arrowHeight).stroke({
          color: obj.axis.color,
          width: obj.axis.width,
          linecap: 'round'
        });
        ...
        obj.svg.line(x, y, x - obj.axis.arrowHeight, y + obj.axis.arrowHeight).stroke({
          color: obj.axis.color,
          width: obj.axis.width,
          linecap: 'round'
        });
  ...
  static addLabels(obj) {
    let y = obj.svg.height() - obj.style.canvas.margin.bottom - this.getLabelsMaxHeight(obj); // Labels over axis arrows
    ...
    if (obj.axis.direction == 'left') {
      ...
      obj.axis.labelBlock[0].svg.x(x).y(y);
      obj.svg.add(obj.axis.labelBlock[0].svg);
    } else if (obj.axis.direction == 'right') {
      let x = obj.svg.width() - obj.style.canvas.margin.right - obj.axis.labelBlock[0].svg.width() / 2;
      ...
      let xRight = obj.svg.width() - obj.style.canvas.margin.right - obj.axis.labelBlock[0].svg.width() / 2;

      for (let i = Columns.count(obj); i > obj.axis.position.right; i--) {
        xRight -= Columns.getMaxWidth(obj, i) + obj.style.columns.interval;
      }

      obj.axis.labelBlock[0].svg.x(xLeft).y(y);
      obj.svg.add(obj.axis.labelBlock[0].svg);
      obj.axis.labelBlock[1].svg.x(xRight).y(y);
    ...
    for (let column of obj.columns) {
      if (column.axisLabel != null) {
        column.axisLabel.svg.x(column.rows[0].block.svg.x() + column.rows[0].block.svg.width() / 2 - column.axisLabel.svg.width() / 2).y(y);
  ...
  static addTicks(obj) {
    ...
    for (let column of obj.columns) {
      if (column.time.text != '') {
        ...
        obj.svg.line(tickX, y - obj.axis.tickHeight, tickX, y + obj.axis.tickHeight).stroke({
          color: obj.axis.color,
          width: obj.axis.width,
          linecap: 'round'
        });
