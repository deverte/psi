  ...
  static arrange(obj) {
    for (let i = 0; i < Columns.count(obj); i++) {
      for (let row of obj.columns[i].rows) {
        ...
        x += (Columns.getMaxWidth(obj, obj.columns[i].n) - row.block.svg.width()) / 2; // Calculate `y` coordinate
        ...
        if (row.type.name == 'controlledInteraction') {
          if (row.n < row.type.controlled) {
            ...
            if (y != undefined) {
              ...
              obj.svg.line(x, y0, x, y).stroke({
                color: ciStyle.strokeColor,
                width: ciStyle.lineWidth,
                linecap: 'round'
              });
              obj.svg.circle(ciStyle.circleDiameter).move(x - ciStyle.circleDiameter / 2, y - ciStyle.circleDiameter / 2).fill(ciStyle.strokeColor);
            }
          }
        }
      }
